...
import tmp from "./results/10Times_tmp_italy.json" with { type: "json" }
...
const main = async () => {
  ...
  for (let i = 0; i < tmp.length; i++) {
    ...
    await writeFile(`results/10Times_full_italy.json`, JSON.stringify(tmp, null, 2));

    console.log("Saving csv...");
    const csv = parse(tmp);
    await writeFile(`results/10Times_full_italy.csv`, csv);
  }
};
