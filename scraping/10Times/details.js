...
import tmp from "./results/10Times_tmp.json" with { type: "json" }
...
const main = async () => {
  ...
  await writeFile(`results/10Times_full.json`, JSON.stringify(tmp, null, 2));

  console.log("Saving csv...");
  const csv = parse(tmp);
  await writeFile(`results/10Times_full.csv`, csv);
  await browser.close();
};
